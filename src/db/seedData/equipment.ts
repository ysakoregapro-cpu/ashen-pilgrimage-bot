...
const SETS: { id: string; name: string; tier: string; bonuses: { count: number; desc: string; effect: Record<string, number> }[] }[] = [
  ...
  { id: 'set_deep_furnace', name: '深層炉シリーズ', tier: 'SSR', bonuses: [
    { count: 2, desc: 'スキルCT軽減 +2%', effect: { skill_cd_reduce: 0.02 } },
    { count: 3, desc: '機械系ブレイク値 +5%', effect: { machine_break_pct: 0.05 } },
    { count: 5, desc: 'ブレイク時追加ダメ', effect: { break_bonus_dmg: 0.15 } },
  ]},
  { id: 'set_black_lamp', name: '黒灯シリーズ', tier: 'SSR', bonuses: [
    { count: 2, desc: '会心ダメ +5%', effect: { crit_damage: 0.05 } },
    { count: 3, desc: '闇属性スキル威力 +5%', effect: { dark_skill_pct: 0.05 } },
    { count: 5, desc: 'HP30%以下で会心率上昇', effect: { low_hp_crit: 0.1 } },
  ]},
  { id: 'set_starfall', name: '星落ちシリーズ', tier: 'SSR', bonuses: [
    { count: 2, desc: '魔力 +4%', effect: { magic_pct: 0.04 } },
    { count: 3, desc: '星属性ダメ +5%', effect: { star_dmg_pct: 0.05 } },
    { count: 5, desc: 'スキル使用時低確率追加星弾', effect: { bonus_star_shot: 0.12 } },
  ]},
  { id: 'set_iron_snow', name: '鉄雪シリーズ', tier: 'SSR', bonuses: [
    { count: 2, desc: '防御 +4%', effect: { defense_pct: 0.04 } },
    { count: 3, desc: '氷属性耐性 +8%', effect: { ice_resist: 0.08 } },
    { count: 5, desc: '防御時味方全体小バリア', effect: { party_barrier: 1 } },
  ]},
  { id: 'set_valhalla', name: 'ヴァルハラシリーズ', tier: 'UR', bonuses: [
    { count: 2, desc: '全ステ +2%', effect: { all_stat_pct: 0.02 } },
    { count: 3, desc: 'レイド中ダメ +5%', effect: { raid_dmg_pct: 0.05 } },
    { count: 5, desc: 'ブレイク中追加ダメ・UR抽選微増', effect: { break_dmg_pct: 0.1, ur_rate_bonus: 0.02 } },
  ]},
  { id: 'set_old_king', name: '旧王シリーズ', tier: 'UR', bonuses: [
    { count: 2, desc: '攻撃 +4%', effect: { attack_pct: 0.04 } },
    { count: 3, desc: '精神攻撃耐性 +8%', effect: { spirit_resist: 0.08 } },
    { count: 5, desc: 'HP50%以下スキル威力上昇・被ダメ増', effect: { low_hp_skill_pct: 0.15, low_hp_taken_pct: 0.1 } },
  ]},
];
